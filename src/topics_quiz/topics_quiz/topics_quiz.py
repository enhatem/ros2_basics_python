...
class TopicsQuiz(Node):
    ...
    def __init__(self):
        ...
        self.dist_travelled = Float64()  # variable to store the distance travelled
        # variable used to store the distance travelled after rotation
        self.dist_travelled_V2 = Float64()
        # in order to store the position after the 90 deg rotation is performed
        self.X0_V2 = Point()
        self.curr_roll = Float64()
        self.curr_pitch = Float64()
        self.curr_yaw = Float64()
        self.curr_quaternion = [0, 0, 0, 1]
        self.bool = False
        self.rotation = False
    ...
    def read_odometry(self, msg):
        ...
        if self.bool is False:
            ...
            self.get_logger().info(f'Initial position coordinated: {self.X0}')
            self.bool = True
